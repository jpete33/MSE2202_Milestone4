/*
 ...
 MSE 2202 MSEBot base code for testing collection
 Language: Arduino
 Authors: Eugen Porter and Michael Naish and Kai Elrick
...
int temp = 0; 
long millisTemp; //temp for recording time
         ...
         case 1: // Run robot
            if (timeUp2sec) {                                                  // pause for 2 sec before running case 1 code
               ...
               if (motorsEnabled) {                                            // run motors only if enabled
                if(temp == 0){    
                  driveIndex = 1;   //start driving forwards 
                  if(pos[0] > 40000){
                    temp = 1;  //once its gone 100cm move to next state 
                  }
                }                       
